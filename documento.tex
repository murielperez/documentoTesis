...
Las Máquinas de Soporte Vectorial (MSV) son sistemas de clasificación que utilizan con conjunto de hipótesis las funciones lineales en un espacio de alta dimensión. Las MSV como se conocen hoy en día fueron propuestas por Cortes y Vapnik en (cita vapnik-cortes) basado en untilizar núcleos para calcular productos internos en espacios de alta dimensión para encontrar planos de margen maximal. Daremos una discusión sobre clasificadores lineales y daremos la teoría básica de MSV. Para saber más, un introducción concisa a MSV puede ser encontrada en (cita cristianini).

Supongamos que tenemos un problema de clasificación binario, es decir, que las posibles clases son dos, digamos $\{-1,1\}$. Una muestra de entrenamiento de tamaño N  $\mathcal{L} = \{(\vec{x}_1, j_1), \dots, (\vec{x}_N, j_N)\}$ es linealmente separable si existe un plano definido por $\langle \vec{w}, \vec{x} \rangle = b$ tal que
\begin{equation}\label{eq:linSep}
j_{i}\langle \vec{w}, \vec{x}_i \rangle +b> 0
\end{equation}
para $i = 1,\dots, N$. $\vec{w}$ es el vector perpendicular al plano definido por $\langle \vec{w}, \vec{x} \rangle = b$ y la condición \ref{eq:linSep} corresponde a que los elementos de la clase $1$ quedan en un semiespacio definido por $\langle \vec{w}, \vec{x} \rangle > 0$, mientras que los pertenecientes a la clase $-1$ quedan en el semiespacio $\langle \vec{w}, \vec{x} \rangle < 0$. En en el caso en que los datos son linealmente separables el plano separador puede ser encontrado mediante el algorítmo del perceptrón de Rossenblat (cita, perceptron). Sin embargo este plano puede no estar bien definido, en el sentido de que puede haber más de un plano separador para un conjunto de datos, por lo que es necessario definir una noción de lo que significa elegir ``el mejor'' plano.
...
\end{aligned}
\end{equation}
Donde la restricción hace referencia a que el margen funcional de cada $\vec{x}_i$ con respecto al plano debe ser mayor a 1.

Hasta ahora solo hemos consierado situaciones en las que los datos $(\vec{x}_i,j_i)$ son linealmente separables. Para permitir clasificaciones erróneas podemos introducir variables de holgura $\xi_i$ en el problema \ref{eq:optimizacionLinSep}
