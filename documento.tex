\documentclass[letterpaper,12pt, draft]{book}
...
\end{equation}
 sin embargo en dimensión $d$ se ha demostrado que estos convergen a una velocidad de $O(N^{1/(4+d)})$\todo{revisar y cita} por lo cual no son prácticos en dimensiones altas. 
...
Las caracterísitcas utilizadas se encuentran en la tabla  \ref{cuadro:variablesUtilizadas}. La media, la desviación estándar, el sesgo,la curtosis, el rango y la cariación cuadrática son variables bien conocidas de estadística descriptiva. El valor Abbe $\mathcal{A}$ y el valor Abbe promedio $\bar{\mathcal{A}}_t$ fueron propuestos para el estudio de fenómenos transientes en \cite{mowlavi_searching_2014} y su definición e interpretación son dadas má adelante. La entropía de Rényi es una medida de la incertidumbre propuesta por \cite{renyi_measures_1961} que generaliza la entropía de Shannon \cite{shannon_mathematical_1948} y que le da más peso a los valores más probables de $m_i$. A continuación discutimos cada una de estas variables.
...
El rango es una medida de la ampliud de las oscilaciones de una curva de luz. Se sabe que a medida que la magnitud promedio de una estrella variable aumenta, aumenta también la amplitud de las oscilaciones. \todo{Información astronómica}

El valor Abbe fue propuesto por \cite{mowlavi_searching_2014} para la detección de estrellas que muestran fenómenos transientes. El valor Abe es el cociente de dos cantidades. Por un lado si la medición $m_i=m(t_i)$ es una versión corrupta de la magnitud verdadera $y(t)$ y $m(t) = y(t)+\epsilon$ con $\epsilon$ una variable aleatoria tal que $E(\epsilon)$, es posible demostrar que 
\begin{equation}
...
\end{equation} 
casi seguramente cuando el número de observaciones tiende a infinito y $y(t)$ es Lypschitz continua y $\epsilon$ ...  \todo{mirar condiciones para que esta convergencia se de}. $y(t)$ puede ser visto como un modelo que será ajustado a los datos por lo que si tomamos el modelo  y $Var(\epsilon)$ como la varianza residual con respecto a este modelo. Estimar la varianza residual con \ref{eq:abbeNumerador} tiene la virtud de que no es necesario postular una forma para $y(t)$ para calcularlo. En el caso de que $y(t)=\mu$, 
\begin{equation}
\frac{1}{n}\sum_i\left(m_i-\mu\right)^2\to Var(\epsilon).
\end{equation} 
...
\end{equation}
es una medida de qué tan lejos está $y(t)$ de ser constante basado en la información disponible en las observaciones $m_i$. En el caso de que $y(t)$ sea una constante debería suceder que $\mathcal{A}\approx 1$ mientras que de lo contrario es de esperar que . Para el caso en que $y(t)$ es constante es de esperar que $\mathcal{A}$ sea cercano a uno, mientras que de lo contrario el se espera que \ref{eq:abbeNumerador} sea menor que \ref{eq:abbeDenominador} y, por lo tanto $\mathcal{A}<1$. 

El valor Abbe $\mathcal{A}$ puede ser calculado en subintervalos de tiempo y puede dar cuenta de la escala de tiempo en la cual una curva de luz cambia. Consideremos un valor fijo $\Delta t$ y para cada instante $t_i$ podemos definir $S_i = \{k|t_k\in(t_i-\Delta t, t_i + \Delta t)\}$. Con esto para cada $t_i$ podemos calcular el valor Abbe utilizando exclusivamente $\{m_k\}_{k\in S_i}$, que son las mediciones hechas en el intervalo de tiempo $(t_i-\Delta t, t_i + \Delta t)$. Llamemos a cada uno de estos valores calculados $\mathcal{A}_{t,i}$ y definamos
\begin{equation}
  \bar{\mathcal{A}}_t = \frac{1}{n}\sum_{i=1}^{n}\mathcal{A}_{t,i}
\end{equation}
donde el subíndice $t$ hace referencia a la escogencia de $\Delta t$ y $n$ es el número de observaciones disponibles para la curva de luz en cuestión.

La entropía de Shannon mide qué tan sorpresiva es una realización de una variable aleatoria y puede ser estimada eficientemente. La entropía es un funcional de la densidad de probabilidad, es convexa, alcanza su máximo cuando la variable aleatoria se distribuye uniformemente y es mínima cuando uno la variable aleatoria toma uno de sus valores con probabilidad 1. Shannon demostró que bajo ciertas condiciones es el único funcional de la densidad de probabilidad que puede dar cuenta la incertidumbre que se tiene ante una realización de la variable aleatoria. En el contexto de comunicaciones, Shannon demostró que si $M$ representa los posibles símbolos que produce una fuente de mensajes, la entropía de $M$, $H(M)$ bits/símbolo es la longitud mínima de un código que represente los mensajes producidos por $M$, es decir, la entropía $H(M)$ es una cota inferior para la compresibilidad del resultado de $M$. Intuitivamente la entropía de una variable que toma pocos valores con gran probabilidad será menor que la de una variable que toma muchos valores con probabilidades parecidas, por ejemplo es de esperar que la entropía la serie de magnitudes de curva de luz como \todo{figura con poca entropía} sea mayor que la de una curva como \todo{figura con mucha entropia}.  Para una variable aleatoria continua $M$ se define como
\begin{equation}
  H(M)=-\int P(M=m)\log_2P(M=m)dm
\end{equation}
sin embargo esta definición tiene propiedades inusuales, pues depende de las coordenadas en que se calcule. Con frecuencia la entropía de Shannon se estima discretizando la variable $M$. Se escoge un número de particiones $n_p$ y se estima la probabilidad de que la variable tome valores en cada partición $p_k$ con el la fracción de los datos que se encuentra en cada partición y se estima
\begin{equation}
  H(M)\approx-\sum_{k=1}^{n_p}p_i\log_2 p_i.
\end{equation}
A esta cantidad nos referimos como entropía de la partición, $H(M,n_p)$. Para realizar estas estimaciones utilizamos $n_p=10$ y la implementación del paquete entropy \todo{citar paquete entropy} para R.

La entropía de Rényi \cite{renyi_measures_1961} es otra medida de la incertidumbre. La entrpía de Renyí de una partición es definida de manera análoga a la entropía de Shannon y es 
\begin{equation}
 H_\alpha(M,n_p) = \frac{1}{1-\alpha}\log_{2}{\sum_{x}p_{i}^{\alpha}}.
\end{equation}
$H_\alpha$ tiene propiedades similares a la entropía de shannon: para $\apha >1$ es una función convexa, adquiere su máximo cuando $M$ se distribuye uniformemente y su mínimo cuando $M$ toma un valor con probabilidad 1. Cuando $\alpha\to 1$, la entropía de Rényi tiende a la entropía de Shannon. 
